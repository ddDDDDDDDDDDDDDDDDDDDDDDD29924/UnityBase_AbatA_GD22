...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (playerStats == null)
            Debug.LogWarning("PlayerStats reference is missing on PlayerController", this);

        if (cameraTransform == null)
            Debug.LogWarning("Camera Transform reference is missing on PlayerController.", this);

        if (InputManager.Instance == null)
            Debug.LogWarning("InputManager instance not found. PlayerController will not receive input.", this);
    }
